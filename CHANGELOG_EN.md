...
- fix: |Deployment| Fix the Pages Functions workflow silently skipping deployment when `PAGE_TOML` is unset, default to the repository `pages/wrangler.toml` service binding, and reuse `FRONTEND_NAME` so the proxy is deployed to the live frontend project; guard admin list responses as arrays to prevent `map` errors
